"""Web UI — Flask app with search, email, and template drafting with edit step."""
...
def _send_smtp(c, to, subject, body, attachment_path=None):
    ...
    if attachment_path and Path(attachment_path).exists():
        ...
        msg.attach(part)
    try:
        with smtplib.SMTP(ec.get("smtp_host","smtp.gmail.com"), int(ec.get("smtp_port",587)), timeout=30) as s:
            s.starttls(); s.login(sender, pw); s.send_message(msg)
        return None
    except Exception as e: return str(e)

def _company_info(c):
    co = c.raw.get("company", {}) or {}
    logo_path = co.get("logo", "")
    if logo_path and not Path(logo_path).is_absolute():
        logo_path = str(ROOT / logo_path)
    return {
        "company":  co.get("name") or c.raw.get("data_store", {}).get("display_name", ""),
        "phone":    co.get("phone", ""),
        "email":    co.get("email") or c.raw.get("email", {}).get("sender", ""),
...
def index():
    ...
    return render_template("index.html",
        company=c.raw.get("company", {}).get("name") or c.raw.get("data_store", {}).get("display_name", "SMB Search"),
...
def api_query():
    ...
    prop, dtype, mode, session = data.get("property") or None, data.get("doc_type") or None, data.get("mode","answer"), data.get("session") or None
    c = cfg()
    try:
        if mode == "search":
            hits = do_search(c, query, property_=prop, doc_type=dtype, page_size=10)
            return jsonify({"mode":"search","results":[{"rank":h.rank,"property":h.property,"doc_type":h.doc_type,"filename":h.filename,"uri":h.uri} for h in hits]})
        a = do_answer(c, query, property_=prop, doc_type=dtype, session=session)
        sources = [{"reference_id":s.get("reference_id",""),"title":s.get("title",""),"property":s.get("property",""),"category":s.get("category",""),"doc_type":s.get("doc_type",""),"uri":s.get("uri","")} for s in a.sources]
        ...
        return jsonify({"mode":"answer","text":text,"sources":sources,"citations":a.citations,"session":a.session})
...
def api_email():
    data = req.get_json(force=True)
    to, docs = data.get("to","").strip(), data.get("docs",[])
    ...
    err = _send_smtp(c, to, f"{len(lines)} documents from AI Search", "Documents:\n\n"+"\n\n".join(lines)+"\n\nLinks expire in 24h.\n")
...
def api_templates():
    ...
    for f in sorted(TEMPLATE_DIR.glob("*.md")):
        ...
        templates.append({"id":f.stem,"title":title,"fields":len(phs)})
    return jsonify({"templates":templates})

@app.route("/api/draft/fill", methods=["POST"])
def api_draft_fill():
    data = req.get_json(force=True)
    tpl_id = data.get("template","").strip()
    ...
    try:
        filled, results = engine.fill(text, query_map)
    except Exception as e:
        return jsonify({"error":str(e)}), 500
    fields = [{"name":r.placeholder,"answer":r.answer,"ok":r.success,"query":r.query} for r in results]
...
def api_draft_pdf():
    ...
    c = cfg()
    brand = _company_info(c)

    from drafting.writer import write_pdf
    fid = uuid.uuid4().hex[:8]
    pdf_name = f"{tpl_id}-{fid}.pdf"
    pdf_path = OUTPUT_DIR / pdf_name
    try:
        write_pdf(text, pdf_path,
                  title=tpl_id.replace("-"," ").title(),
    ...
    if to_email:
        subject = f"{tpl_id.replace('-',' ').title()} - {prop or 'Report'}"
        err = _send_smtp(c, to_email, subject, f"Attached: {subject}\n", str(pdf_path))
